 ...
 * (yesterday .. today + 7). Ids are deterministic so re-running is idempotent and
 * never creates duplicate doses.
 */
@Singleton
class EventGenerator @Inject constructor(
    ...
    suspend fun refreshMedicine(medicine: MedicineEntity) {
        ...
        val existing = events.forMedicineBetween(medicine.id, from.format(fmt), to.format(fmt)).associateBy { it.id }

        val toInsert = expected.filter { it.id !in existing }.map { d ->
            EventEntity(
        ...
        for (e in existing.values) {
            ...
            val d = expectedById[e.id]
            when {
                // The schedule no longer produces this dose: retire future, not-yet-acted-on doses.
                d == null && !terminal && e.scheduledAt > now && !e.deleted -> {
                    val gone = e.copy(deleted = true, updatedAt = now)
                    events.upsert(gone)
                    outbox.event(gone.toDto())
                }
                // A retired dose is back in the schedule.
                d != null && e.deleted && e.scheduledAt > now -> {
                    val back = e.copy(deleted = false, doseAmount = d.doseAmount, doseUnit = d.doseUnit, scheduledAt = d.scheduledAt.toEpochMilli(), zoneId = zone.id, updatedAt = now)
                    events.upsert(back)
                    outbox.event(back.toDto())
                }
                // Dose amount or instant changed for a pending dose.
                d != null && !terminal && !e.deleted && (d.doseAmount != e.doseAmount || d.doseUnit != e.doseUnit || d.scheduledAt.toEpochMilli() != e.scheduledAt) -> {
                    val upd = e.copy(doseAmount = d.doseAmount, doseUnit = d.doseUnit, scheduledAt = d.scheduledAt.toEpochMilli(), zoneId = zone.id, updatedAt = now)
                    events.upsert(upd)
                    if (d.doseAmount != e.doseAmount || d.doseUnit != e.doseUnit) outbox.event(upd.toDto())
                }
            }
        }
    }
