...
/*
 * Jadi-Buti design system: one header style, one card style, one chip style, big buttons.
 * Everything is sized for elderly users: 56–60dp touch targets, 16–18sp body text.
 */
...
fun BigOutlinedButton(text: String, onClick: () -> Unit, modifier: Modifier = Modifier, enabled: Boolean = true, icon: ImageVector? = null) {
    OutlinedButton(onClick = onClick, enabled = enabled, modifier = modifier.fillMaxWidth().heightIn(min = 56.dp), shape = RoundedCornerShape(16.dp)) {
        if (icon != null) { Icon(icon, contentDescription = null, modifier = Modifier.size(24.dp)); Spacer(Modifier.size(10.dp)) }
        Text(text, style = MaterialTheme.typography.labelLarge)
    }
}

/** Gradient header used on every screen. Optional subtitle and trailing actions. */
@Composable
fun AppTopBar(title: String, subtitle: String? = null, onBack: (() -> Unit)? = null, actions: @Composable RowScope.() -> Unit = {}) {
    Box(Modifier.fillMaxWidth().background(Brush.horizontalGradient(listOf(GreenDark, Green)))) {
...
fun SectionHeader(text: String, modifier: Modifier = Modifier, icon: ImageVector? = null, trailing: (@Composable () -> Unit)? = null) {
    Row(modifier.fillMaxWidth().padding(horizontal = 4.dp, vertical = 8.dp), verticalAlignment = Alignment.CenterVertically) {
        if (icon != null) { Icon(icon, contentDescription = null, tint = MaterialTheme.colorScheme.primary, modifier = Modifier.size(24.dp)); Spacer(Modifier.width(8.dp)) }
...
fun AppCard(modifier: Modifier = Modifier, container: Color = MaterialTheme.colorScheme.surface, padding: Int = 16, content: @Composable () -> Unit) {
    Card(modifier = modifier.fillMaxWidth(), shape = RoundedCornerShape(20.dp), colors = CardDefaults.cardColors(containerColor = container), elevation = CardDefaults.cardElevation(defaultElevation = 1.dp)) {
...
fun toneColors(tone: Tone): Pair<Color, Color> = when (tone) {
    ...
    Tone.AMBER -> AmberSoft to Color(0xFF8A5A00)
    Tone.RED -> RedSoft to Red
    Tone.BLUE -> BlueSoft to Blue
    Tone.GREY -> Color(0xFFEEF1EF) to Grey
}
...
fun MemberAvatar(name: String, size: Int = 44) {
    Box(Modifier.size(size.dp).background(Brush.linearGradient(listOf(Green, GreenDark)), CircleShape), contentAlignment = Alignment.Center) {
        Text(name.trim().take(1).uppercase(), color = Color.White, style = MaterialTheme.typography.titleLarge)
    }
}
...
fun InfoBanner(text: String, tone: Tone = Tone.AMBER, modifier: Modifier = Modifier, icon: ImageVector? = null, action: (@Composable () -> Unit)? = null) {
    ...
    Surface(color = bg, shape = RoundedCornerShape(16.dp), modifier = modifier.fillMaxWidth()) {
        Row(Modifier.padding(14.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(text, style = MaterialTheme.typography.bodyMedium, modifier = Modifier.weight(1f), color = MaterialTheme.colorScheme.onSurface)
...
fun LabelledProgress(label: String, progress: Float, tone: Tone = Tone.GREEN, modifier: Modifier = Modifier) {
    ...
    Column(modifier.fillMaxWidth()) {
        ...
        LinearProgressIndicator(progress = { progress.coerceIn(0f, 1f) }, modifier = Modifier.fillMaxWidth().height(12.dp), color = fg, trackColor = fg.copy(alpha = 0.18f))
    }
}
...
fun StatTile(value: String, label: String, tone: Tone, modifier: Modifier = Modifier) {
    val (bg, fg) = toneColors(tone)
    Surface(color = bg, shape = RoundedCornerShape(16.dp), modifier = modifier) {
